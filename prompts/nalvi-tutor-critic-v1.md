...
Reject `AUDIO_SELECT` unless all of `audioId`, relative `audioPath`, `audioText`, `audioAuthorized: true`, `humanRecorded: true`, and `audioSource: "manifest-human-recording"` match the single whitelisted entry in `approvedActivityMaterial.audio`, and `correctAnswer` matches either that canonical `audioText` or only its documented base before a parenthetical gloss. Reject every non-audio activity unless its four audio strings are empty and both audio booleans are false. Never infer authorization from a boolean, path, text, or client claim. Never approve invented pairs, categories, dialogue turns/options, contexts, audio references, or generic text-to-speech as recorded evidence.
