...
const PRODUCTIVE_STATUS = "expertVerified";
...
const sourceStatusResult = (record, openConflictIds = []) => {
  ...
  if (record.validationStatus !== PRODUCTIVE_STATUS) {
...
const validatedSubrecord = record =>
  record?.validationStatus === PRODUCTIVE_STATUS &&
...
export const compileKnowledgeBase = ({corpus, governance}) => {
  ...
  return {
    ...
    validationPolicy: {
      requiredValidationStatus: governance?.generationGate?.requiredValidationStatus || PRODUCTIVE_STATUS,
...
export const createGrammarEngine = ({corpus, governance}) => {
  ...
  const getValidatedVerbForm = (verbId, grammaticalPerson, options = {}) => {
    ...
    return {
      ...
      validationStatus: PRODUCTIVE_STATUS,
      canResolveWithoutAI: true,
      aiPermitted: false,
      verbId,
      patternId: pattern.id
  ...
  return Object.freeze({
    ...
    policy: Object.freeze({
      ...
      productiveValidationStatus: PRODUCTIVE_STATUS,
