...
(function(){
  ...
  const VERSION="NALVI-NOTIFICATION-CENTER-2";
  const COPY={
    es:{label:"Notificaciones",title:"Marandu · Notificaciones",empty:"Todavía no hay interacciones nuevas.",signIn:"Inicia sesión para ver tus notificaciones.",comment:"comentó en tu publicación",like:"marcó Me gusta en tu publicación",follow:"comenzó a seguirte",message:"te envió un mensaje",someone:"Alguien",open:"Abrir",close:"Cerrar"},
    en:{label:"Notifications",title:"Marandu · Notifications",empty:"There are no new interactions yet.",signIn:"Sign in to see your notifications.",comment:"commented on your post",like:"liked your post",follow:"started following you",message:"sent you a message",someone:"Someone",open:"Open",close:"Close"},
    pt:{label:"Notificações",title:"Marandu · Notificações",empty:"Ainda não há novas interações.",signIn:"Entre para ver suas notificações.",comment:"comentou na sua publicação",like:"curtiu sua publicação",follow:"começou a seguir você",message:"enviou uma mensagem para você",someone:"Alguém",open:"Abrir",close:"Fechar"},
    fr:{label:"Notifications",title:"Marandu · Notifications",empty:"Il n’y a pas encore de nouvelles interactions.",signIn:"Connectez-vous pour voir vos notifications.",comment:"a commenté votre publication",like:"a aimé votre publication",follow:"a commencé à vous suivre",message:"vous a envoyé un message",someone:"Quelqu’un",open:"Ouvrir",close:"Fermer"},
    it:{label:"Notifiche",title:"Marandu · Notifiche",empty:"Non ci sono ancora nuove interazioni.",signIn:"Accedi per vedere le notifiche.",comment:"ha commentato il tuo post",like:"ha messo Mi piace al tuo post",follow:"ha iniziato a seguirti",message:"ti ha inviato un messaggio",someone:"Qualcuno",open:"Apri",close:"Chiudi"},
    de:{label:"Benachrichtigungen",title:"Marandu · Benachrichtigungen",empty:"Es gibt noch keine neuen Interaktionen.",signIn:"Melde dich an, um Benachrichtigungen zu sehen.",comment:"hat deinen Beitrag kommentiert",like:"gefällt dein Beitrag",follow:"folgt dir jetzt",message:"hat dir eine Nachricht gesendet",someone:"Jemand",open:"Öffnen",close:"Schließen"}
  };
  ...
  function itemCopy(item){const c=copy(),actor=escapeHtml(item.actorName||c.someone),action=escapeHtml(c[item.kind]||c.comment),body=(item.kind==="comment"||item.kind==="message")&&item.body?`<small>“${escapeHtml(item.body)}”</small>`:"",post=item.postBody?`<em>${escapeHtml(item.postBody)}</em>`:"",icon=item.kind==="comment"?"💬":item.kind==="message"?"✉":item.kind==="like"?"♥":"＋";return `<span class="nalvi-notification-kind ${escapeHtml(item.kind)}" aria-hidden="true">${icon}</span><span><strong>${actor} ${action}</strong>${body}${post}<time>${escapeHtml(item.date||"")}</time></span>`}
  function render(){
